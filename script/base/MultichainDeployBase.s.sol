...
contract MultichainDeployBaseScript is Script {
    function deployToChain(string memory network, string memory tag, bytes memory bytecode) internal {
    ...
    function deployToChains(string[] memory networks, string memory tag, bytes memory bytecode) internal {
        for (uint256 i; i < networks.length; i++) {
            deployToChain(networks[i], tag, bytecode);
        }
    }
}